...
short_description: This module is used for generating the Auth_type for access_interfaces if auth_type is present in host_vars/access_intf folder,else it will take from the auth_db.yml by taking diff from the leaf configs and return the modified dictionary
'''
      ...
      for access_intf in access_input['access_interfaces']['access'] :
          if isinstance(access_intf,dict) :
            for keys,values in access_intf.items():
              if 'auth_type' in values and 'source_template' not in parsed_sec_output['interfaces'][keys]:
                auth_dict[keys] = values['auth_type']
              elif 'leaf_auth_type' in access_input and 'source_template' not in parsed_sec_output['interfaces'][keys] :
                auth_dict[keys] = access_input['leaf_auth_type']
              elif 'site_level_fabric_auth_type' in userinput and 'source_template' not in parsed_sec_output['interfaces'][keys] :
