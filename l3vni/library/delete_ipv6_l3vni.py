...
short_description: This module is used for deleting the ipv6 values that are provisioned in the device by comparing all.yml and show run nve parser  
...
def compare(userinput:dict,parsed_output:dict, overlay_db:dict, leaf_data:dict )->dict:
  for vrf in userinput['l3vni'] :
    if vrf == "all" :
      for overlay_db_vrf in overlay_db['vrfs'].keys() :
        if 'ipv6' in parsed_output['vrf'][overlay_db_vrf]['address_family'].keys() :
          mul_list_dict['dag'].append(overlay_db_vrf)
    else :
      if 'ipv6' in parsed_output['vrf'][vrf]['address_family'].keys() :
  ...
  if vrfs_dict and svis_dict :  
    yml_dict =  {'ipv6_action': 'delete' ,'ipv6_routing' : 'yes' , 'vrfs' : vrfs_dict, 'bgp' : {'as_number' : int(leaf_data['bgp']['as_number'])} , 'svis' : svis_dict }
  else :
    yml_dict = {}
