...
def dhcp_del_dict(to_del, overlay_dict, header_output):
    ...
    if 'all' in to_del['vrfs']:
        del_header = True
        to_del['vrfs'] = list(overlay_dict['vrf'])
    elif sorted(to_del['vrfs']) == sorted(list(overlay_dict['vrf'])):
        del_header = True
    ...
    for svi, svi_val in dev_svis_dict.items():
        if svi_val['svi_type'] == 'access' and svi_val.get('vrf') in to_del['vrfs']:
